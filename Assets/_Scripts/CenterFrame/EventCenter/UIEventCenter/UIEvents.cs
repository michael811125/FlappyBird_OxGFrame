...
public class EDescriptionUI : EventBase
{
    ...
    {
        Logging.Print<MainLogger>($"<color=#FFC078>【Handle Event】 -> {nameof(EDescriptionUI)}</color>");

        await CoreFrames.UIFrame.Show<DescriptionUI>(UIs.DescriptionUI, new object[] { this._msg, this._closeAction }, UIs.AwaitingUI, 0);
...
public class EDoubleCheckUI : EventBase
{
    ...
    public async override UniTaskVoid HandleEvent()
    {
        Logging.Print<MainLogger>($"<color=#FFC078>【Handle Event】 -> {nameof(EDoubleCheckUI)}</color>");

        await CoreFrames.UIFrame.Show<DoubleCheckUI>(UIs.DoubleCheckUI, new object[] { this._title, this._msg, this._yesAction, this._noAction }, UIs.AwaitingUI, 0);
